...
get_config_value() {
    ...
    if [ "$GET_SECURITY_GROUP_ID" -eq 1 ]; then
        cat "${REGIONS_CONFIG_FILE}" | jq ".[] | select(.region == \"$region\") | to_entries[] | select(.key == \"security-group-id\") | .value" | tr -d '"'
    elif [ "$GET_SUBNET" -eq 1 ]; then
        # First check if there are any subnets defined before we try to index
        subnet_map=$(cat "${REGIONS_CONFIG_FILE}" | jq ".[] | select(.region == \"${region}\") | .subnets")
        if [[ -z "${subnet_map}" || "${subnet_map}" == "null" ]]; then
            ...
            echo "$subnet_map" | jq "to_entries[] | select(.key == \"${AVAILABILITY_ZONE}\") | .value" | tr -d '"'
        fi
    elif [ "$GET_AMI_ID" -eq 1 ]; then
        cat "${REGIONS_CONFIG_FILE}" | jq ".[] | select(.region == \"$region\") | to_entries[] | select(.key == \"ec2-ami\") | .value" | tr -d '"'
    fi
}
